...
struct Graph{
	...
	pair<Long, Long> dijkstra(Long s, Long t, Long n) {
		...
		Long cur = t;
		while (true) {
			adj[parent[cur]][parentInd[cur]].flow += cf;
			int to = adj[parent[cur]][parentInd[cur]].to;
			int indTo = indRev[parent[cur]][parentInd[cur]];
			adj[to][indTo].flow -= cf;
			cur = adj[parent[cur]][parentInd[cur]].from;
			if(cur == s) break;
		}
		return {cf, pot[t] * cf};
	}
