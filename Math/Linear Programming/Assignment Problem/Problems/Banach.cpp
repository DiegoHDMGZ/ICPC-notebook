...
//https://codeforces.com/group/Ohoz9kAFjS/contest/266572/problem/K

typedef long long Long;
typedef vector<vector<Long>> Matrix;
const Long INF = 1e18;
const Long MX = 1e3;
const Long dummy = -1e9; 
...
int main() {
	...
	REP(i , n){
		for(Long j = i + 1; j < n; j++){
			...
			Long a = hg.matchRow[i];
			Long b = hg.matchRow[j];
			Long d2 = dist(P[i] + V[a] , P[j] + V[b]);
			if(d1 > d2){
				cout << "No\n";
				return 0;
			}
