...
struct Query {
    ...
    bool operator<(const Query &other) const {
        ...
        return make_pair(make_pair(u, v), l) < make_pair(make_pair(other.u, other.v), other.l);
    }
};
...
namespace DynamicConnectivity {
    vector<Query> transform(vector<Query> &queries) { // O(Q log Q)
        ...
        while (i < queries.size()) {
            if (queries[i].type == QUERY) {
                ...
                newQuery.type = RANGE;
                if (i + 1 < queries.size() && queries[i + 1].u == queries[i].u &&
                    queries[i + 1].v == queries[i].v) {
    ...
    void process(int l, int r, vector<Query> &queries,
                 vector<Long> &answer) { // O(Q log Q log n)
        dsu.save();
        vector<Query> left;
        vector<Query> right;
        int mid = (l + r) / 2;
