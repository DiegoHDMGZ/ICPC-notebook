...
struct Mo {
    int getBlock(int i) { return i / SZ_BLOCK; }

    int lowerLim(int i) { return getBlock(i) * SZ_BLOCK; }

    int upperLim(int i, int sz) { return min(lowerLim(i) + SZ_BLOCK - 1, sz - 1); }

    vector<int> process(int n, vector<Edge> &edges, vector<Query> &queries) {
        // O(M sqrt(Q) log N)
        ...
        while (q < queries.size()) {
            ...
            while (q < queries.size() && curBlock == getBlock(queries[q].l)) {
                if (getBlock(queries[q].l) == getBlock(queries[q].r)) {
                    ...
                    dsu.save();
                    curR = queries[q].r + 1;
                    int lim = upperLim(queries[q].l, edges.size());
                    for (int i = lim; i >= queries[q].l; i--) {
                        dsu.join(edges[i].u, edges[i].v);
                    }
                    answer[queries[q].id] = dsu.getComponents();
                }
